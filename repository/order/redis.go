...
import (
	...
	"github.com/muhimron90/orders-api/model"
...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	resp := txn.SetNX(ctx, key, string(data), 0)
	...
	if err := txn.SAdd(ctx, "orders", key).Err(); err != nil {
		txn.Discard()
		return fmt.Errorf("failed to add order set: %w", err)
	}
	if _, err := txn.Exec(ctx); err != nil {
		return fmt.Errorf("an error while executing add :%w", err)
	}
	return nil
}

var ErrNotExist = errors.New("an order does not exist")
...
func (r *RedisRepo) DeleteById(ctx context.Context, id uint64) error {
	...
	if _, err := txn.Exec(ctx); err != nil {
		return fmt.Errorf("failed while executing delete : %w", err)
	}
	return nil
}
...
func (r *RedisRepo) findAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	...
	if err != nil {
		return FindResult{}, fmt.Errorf("failed to get an order ids : %w", err)
	}
	if len(keys) == 0 {
		return FindResult{
			Orders: []model.Order{},
